function loadStudentSchedule(){let e=new Date;renderStudentCalendar(e.getFullYear(),e.getMonth()+1)}function renderStudentCalendar(e,t){fetch(`student_get_schedule.php?year=${e}&month=${t}&timezone=${STUDENT_TIMEZONE}`).then(e=>{if(!e.ok)throw Error("Ошибка сервера");return e.json()}).then(a=>{if(!Array.isArray(a))throw Error("Неверный формат данных");let l=new Date(e,t-1,1),s=new Date(e,t,0),n=new Date(l);n.setDate(n.getDate()-(0===l.getDay()?6:l.getDay()-1));let i=new Date(s);i.setDate(i.getDate()+(0===i.getDay()?0:7-i.getDay()));let o="";for(let d=new Date(n);d<=i;d.setDate(d.getDate()+1)){let c=d.getFullYear(),r=String(d.getMonth()+1).padStart(2,"0"),u=String(d.getDate()).padStart(2,"0"),m=`${c}-${r}-${u}`,p=a.find(e=>e.lesson_date===m),h=d.getMonth()!==t-1,v=m===new Date().toISOString().split("T")[0];o+=`<div class="calendar-day ${h?"other-month":""} ${v?"today":""}" onclick="${p?`viewLesson(${p.id})`:""}">
    ...
    `}</div>`;let i=a;null!==e&&"all"!==e&&(i=a.filter(t=>t.category_id==e));let o={},d=[];l.forEach(e=>{if(e.block_id){let t=i.some(t=>t.id==e.block_id);t?(o[e.block_id]||(o[e.block_id]=[]),o[e.block_id].push(e)):d.push(e)}else d.push(e)});let c="";i.forEach(e=>{let t=o[e.id]||[];c+=`
            ...
            </div>`),0===i.length&&(null!==e||0===d.length)&&(c+='<div class="empty-state"><div class="empty-icon">\uD83D\uDCDD</div><h3>Нет заданий в этой категории</h3></div>'),n+=c,document.getElementById("mainContent").innerHTML=n}function loadStudentBlocks(e){if("lecture"===e){loadStudentLectures();return}fetch(`student_get_blocks.php?type=${e}`).then(e=>e.json()).then(async t=>{let a=`<h2>Шпоры</h2>`;if(0===t.length)a+=`<div class="empty-state"><div class="empty-icon">📋</div><h3>Нет блоков</h3></div>`;else{for(let l of(a+='<div class="blocks-grid">',t)){let s=await fetch(`student_get_block_items.php?block_id=${l.id}`).then(e=>e.json()),n="";s.length>0?(n='<ul class="block-items-list">',s.forEach(e=>{n+=`<li>${e.title} ${e.link?`<a href="${e.link}" target="_blank">🔗</a>`:""}${e.comment?`<br><small>${e.comment}</small>`:""}</li>`}),n+="</ul>"):n=`<p class="block-empty-text">Нет материалов</p>`,a+=`
                        <div class="block-card" style="cursor:pointer;" onclick="viewStudentBlock(${l.id}, '${l.name.replace(/'/g,"\\'")}', '${e}')">
                            <h3 class="block-card__title">${l.name}</h3>
                            ${n}
                        </div>`}a+="</div>"}document.getElementById("mainContent").innerHTML=a})}function loadStudentLectures(){Promise.all([fetch("student_get_lecture_sections.php").then(e=>e.json()),fetch("student_get_blocks.php?type=lecture").then(e=>e.json())]).then(([e,t])=>{window.currentStudentLectureData={sections:e,blocks:t},renderStudentLectureTabs(null)}).catch(e=>{console.error("Ошибка загрузки лекций:",e),document.getElementById("mainContent").innerHTML='<div class="empty-state"><div class="empty-icon">\uD83D\uDCDA</div><h3>Ошибка загрузки</h3></div>'})}function renderStudentLectureTabs(e=null){let{sections:t,blocks:a}=window.currentStudentLectureData,l="";t.forEach(t=>{let a=e==t.id?"active":"";l+=`<button class="schedule-mode-btn ${a}" onclick="renderStudentLectureTabs(${t.id})">📁 ${t.name}</button>`}),l=`
        <button class="schedule-mode-btn ${null===e?"active":""}" onclick="renderStudentLectureTabs(null)">📁 Все</button>
        ${l}
    `;let s=a;null!==e&&(s=a.filter(t=>t.section_id==e));let n="<h2>Лекции</h2>";n+=`<div class="schedule-mode-switcher" style="margin-bottom:20px;">${l}</div>`,0===s.length?n+='<div class="empty-state"><div class="empty-icon">\uD83D\uDCDA</div><h3>Нет лекций</h3></div>':(n+='<div class="blocks-grid">',s.forEach(e=>{n+=`
                <div class="block-card" style="cursor:pointer;" onclick="viewStudentBlock(${e.id}, '${e.name.replace(/'/g,"\\'")}', 'lecture')">
                    <h3 class="block-card__title">${e.name}</h3>
                    <p class="block-empty-text">Нажмите, чтобы открыть</p>
                </div>`}),n+="</div>"),document.getElementById("mainContent").innerHTML=n}function viewStudentBlock(e,t,a){fetch(`student_get_block_items.php?block_id=${e}`).then(e=>e.json()).then(e=>{let l="";e.forEach(e=>{l+=`
