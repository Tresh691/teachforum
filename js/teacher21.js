    ...
    `}</div>`,n=l;null!==e&&"all"!==e&&(n=l.filter(t=>t.section_id==e)),0===n.length?a+='<div class="empty-state"><div class="empty-icon">\uD83D\uDCD6</div><h3>Нет блоков</h3></div>':(a+=`<div class="blocks-grid" id="libraryBlocksContainer" data-section-id="${e||""}">`,n.forEach(e=>{a+=renderLibraryBlockCard(e)}),a+="</div>"),setMainContent(a),initLibraryDragDrop()}let currentLibrarySectionId=null;function renderLibraryBlockCard(e){return`
        ...
        `}</div>`,n=l;if(null!==e&&(n=l.filter(t=>t.section_id==e)),0===n.length)a+='<div class="empty-state"><div class="empty-icon">\uD83D\uDCDA</div><h3>Нет лекций</h3></div>';else{for(let s of(a+='<div class="blocks-grid">',n)){let i="";try{let r=await fetch(`get_block_items.php?block_id=${s.id}`),d=await r.json();d.length>0?(i='<ul class="block-items-list">',d.forEach(e=>{i+=`<li>${e.title} ${e.link?`<a href="${e.link}" target="_blank" onclick="event.stopPropagation()">🔗</a>`:""}</li>`}),i+="</ul>"):i='<p class="block-empty-text">Нет материалов</p>'}catch(c){i='<p class="block-empty-text">Ошибка загрузки</p>'}a+=`
                    <div class="block-card" data-block-id="${s.id}" style="cursor:pointer;" onclick="openBlockView(${s.id}, '${s.name.replace(/'/g,"\\'")}', 'lecture')">
                        ...
                        ${i}
                    </div>`}a+="</div>"}setMainContent(a)}catch(p){setMainContent('<div class="empty-state"><div class="empty-icon">\uD83D\uDCDA</div><h3>Ошибка загрузки</h3></div>')}}function renderLectureBlockCard(e){return`
    ...
    <div class="block-card" data-block-id="${n.id}" onclick="openBlockView(${n.id}, '${n.name.replace(/'/g,"\\'")}', '${e}')">
        ...
        <div class="help-page">
            ...
            <div style="display:grid; gap:20px;">
                ...
                <!-- 4. Статистика по оплате (в календаре ученика) -->
                ...
                <div style="background:#f8fafc; border-radius:16px; padding:24px; border-left:5px solid #3b82f6;">
                    <div style="font-size:36px; margin-bottom:10px;">📊</div>
                    <h3>4а. Статистика (общая)</h3>
                    <p>В боковом меню появился раздел \xabСтатистика\xbb. В нём две вкладки: \xabОплаты\xbb и \xabПробники\xbb.</p>
                    <p><strong>Оплаты:</strong> показывает суммы за каждый месяц текущего года по статусам: оплачено, не оплачено, ожидается. Внизу итог за год. Суммы считаются по ставкам учеников и их индивидуальным статусам в уроках.</p>
                    <p><strong>Пробники:</strong> список учеников с количеством пробников и средним баллом. При клике на ученика открывается его список пробников с названием, датой, комментарием и баллами. Можно добавлять, редактировать и удалять пробники. Внизу выводится средний балл по всем пробникам ученика.</p>
                    <p>Ученик видит только свои пробники и свой средний балл.</p>
                </div>
                ...
                <div style="background:#f8fafc; border-radius:16px; padding:24px; border-left:5px solid #06b6d4;">
                    ...
                    <p><strong>Разделы:</strong> лекции теперь можно организовать по разделам, как в библиотеке. Нажмите \xab+ Раздел\xbb, чтобы создать новый раздел. Блоки можно перемещать между разделами через редактирование блока (кнопка ✏️) и выбор нужного раздела в выпадающем списке. Удаление раздела не удаляет блоки, они становятся без раздела.</p>
