function renderStudents(){fetch("get_students.php").then(e=>e.json()).then(e=>{let t="";if(!Array.isArray(e)||0===e.length){t='<div class="empty-state"><div class="empty-icon">\uD83D\uDC65</div><h3>Нет учеников</h3><p>Нажмите \xabДобавить ученика\xbb, чтобы создать первого</p></div>',setMainContent(t+='<div style="text-align:center; margin-top:16px;"><button class="btn-add" onclick="openAddStudentModal()">+ Добавить ученика</button></div>');return}let l="";e.forEach(e=>{let t=void 0!==e.rate&&null!==e.rate?e.rate:0;l+=`
    ...
    <div class="block-card" data-block-id="${n.id}" onclick="openBlockView(${n.id}, '${n.name.replace(/'/g,"\\'")}', '${e}')">
        ...
        ${i}
    </div>`}a+="</div>"}setMainContent(a),o.length>0&&initLectureDragDrop(e)}).catch(()=>showEmptyState(l,t,"Ошибка загрузки."))}function openAddBlockModal(e,t=null){let l=document.createElement("div");l.className="modal-overlay active",l.innerHTML=`
        <div class="modal">
            <button class="modal__close" onclick="this.closest('.modal-overlay').remove()">&times;</button>
            <h3>Добавить блок</h3>
            <div class="form-group"><label class="form-label">Название блока</label><input type="text" id="blockName" class="form-input"></div>`,"lecture"===e&&(l.innerHTML+=`
            <div class="form-group">
                <label class="form-label">Раздел</label>
                <select id="blockSection" class="form-select"></select>
            </div>`),l.innerHTML+=`<button class="btn btn--primary" onclick="addBlock('${e}')">Создать</button>`,document.body.appendChild(l),"lecture"===e&&fetch("get_lecture_sections.php").then(e=>e.json()).then(e=>{let l='<option value="">Без раздела</option>';e.forEach(e=>{let o=e.id==t?"selected":"";l+=`<option value="${e.id}" ${o}>${e.name}</option>`}),document.getElementById("blockSection").innerHTML=l})}function addBlock(e){let t=document.getElementById("blockName").value.trim();if(!t)return alert("Введите название");let l=`name=${encodeURIComponent(t)}&type=${e}`;if("lecture"===e){let o=document.getElementById("blockSection"),a=o?o.value:"";l+=`&section_id=${a}`}fetch("add_block.php",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:l}).then(e=>e.json()).then(t=>{t.success?(document.querySelector(".modal-overlay").remove(),"lecture"===e?renderLectures(window.currentLectureSectionId||null):renderBlocks(e)):alert(t.error)})}function openBlockView(e,t,l){fetch(`get_block_items.php?block_id=${e}`).then(e=>e.json()).then(o=>{let a="";o.forEach(o=>{a+=`
                    <tr style="cursor:pointer;" onclick="openLessonView(${o.id}, '${o.title.replace(/'/g,"\\'")}', '${(o.link||"").replace(/'/g,"\\'")}', '${(o.comment||"").replace(/'/g,"\\'")}', ${e}, '${t.replace(/'/g,"\\'")}', '${l}')">
                        <td>${o.title}</td>
                    </tr>`});let n;setMainContent(`
                <button class="btn-back" onclick="${n="lecture"===l?"renderLectures(window.currentLectureSectionId || null)":`renderBlocks('${l}')`}">Назад к блокам</button>
                <div class="dashboard-header">
                    <h2>${t}</h2>
                    <div style="display:flex; gap:8px; align-items:center;">
...
Пароль: ${t.password}`};navigator.share?navigator.share(e).catch(()=>{}):copyToClipboard(e.text,this)}),e.textContent="Закрыть",e.disabled=!1,e.onclick=function(){let t=e.closest(".modal-overlay");t&&t.remove(),renderStudents()}}else alert("Ошибка: "+(t.error||"Неизвестная ошибка")),e.disabled=!1,e.textContent="Создать"}).catch(t=>{alert("Ошибка сети: "+t),e.disabled=!1,e.textContent="Создать"})}function deleteStudent(e,t){confirm(`Удалить ученика ${t}?`)&&fetch("delete_student.php",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:`student_id=${e}`}).then(e=>e.json()).then(e=>e.success?renderStudents():alert(e.error))}function changeMonth(e,t,l,o){renderCalendar(e,t,l,o),loadLessonsTable(e,t,l,o)}function switchViewMode(e,t,l,o){currentViewMode="calendar"===currentViewMode?"table":"calendar";let a=document.getElementById("switchCalendar"),n=document.getElementById("switchTable"),s=document.getElementById("calendarPanel"),i=document.getElementById("tablePanel");if("calendar"===currentViewMode)a.classList.add("active"),n.classList.remove("active"),s.style.display="",i.style.display="none";else{n.classList.add("active"),a.classList.remove("active"),s.style.display="none",i.style.display="";let r=document.getElementById("lessonsTableContainer");r&&(r.innerHTML='<div style="text-align:center;padding:40px;"><div class="loading-spinner"></div></div>'),loadLessonsTable(e,t,l,o)}}function loadLessonsTable(e,t,l,o){fetch(`get_schedule.php?student_id=${e}&month=${o}&year=${l}&timezone=${TEACHER_TIMEZONE}`).then(e=>e.json()).then(a=>{let n=document.getElementById("lessonsTableContainer");if(!n)return;let s=["Январь","Февраль","Март","Апрель","Май","Июнь","Июль","Август","Сентябрь","Октябрь","Ноябрь","Декабрь"][o-1];if(!Array.isArray(a)||0===a.length){n.innerHTML=`
                ...
                `;return}a.sort((e,t)=>(e.lesson_date+e.time).localeCompare(t.lesson_date+t.time));let i="";a.forEach(a=>{let n=a.time?a.time.slice(0,5):"",s=(a.students&&a.students.find(t=>t.student_id==e))?.payment_status||"none",r=getPaymentLabel(s),d={...a,students:a.students||[],payment_status:s};i+=`
                    <tr style="cursor:pointer;"
                        ...
                        <td><span class="badge badge--${s||"none"} badge--clickable" onclick="event.stopPropagation(); changePaymentStatusInTable(${a.id}, ${e}, this)">${r}</span></td>
                        <td>${a.comment||""}</td>
                ...
                </div>`;n.innerHTML=r}).catch(()=>{let e=document.getElementById("lessonsTableContainer");e&&(e.innerHTML="")})}function changePaymentStatusInTable(e,t,l){let o=l.textContent.trim(),a=["none","paid","unpaid","pending"],n=(a.indexOf({"Не указан":"none",Оплачено:"paid","Не оплачено":"unpaid",Ожидается:"pending"}[o]||"none")+1)%a.length,s=a[n];fetch("update_lesson_student_payment.php",{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:`lesson_id=${e}&student_id=${t}&status=${s}`}).then(e=>e.json()).then(e=>{if(e.success){l.textContent=getPaymentLabel(s),l.className=`badge badge--${s} badge--clickable`;let o=l.closest("tr");if(o){let a=JSON.parse(o.dataset.lesson);if(a.students){let n=a.students.find(e=>e.student_id==t);n&&(n.payment_status=s),o.dataset.lesson=JSON.stringify(a)}}refreshStatsFromTable()}else alert(e.error||"Ошибка сохранения статуса")}).catch(e=>{console.error("Ошибка:",e),alert("Ошибка сети")})}function refreshStatsFromTable(){let e=document.querySelectorAll("#lessonsTableContainer tbody tr"),t=0,l=0,o=0,a=0;e.forEach(e=>{let n=JSON.parse(e.dataset.lesson),s=e.dataset.studentId;if(n.students&&n.students.length>0){let i=n.students.find(e=>e.student_id==s);if(i){t++;let r=i.payment_status;"paid"===r?l++:"unpaid"===r?o++:"pending"===r&&a++}}});let n=document.querySelectorAll(".stats-panel__value");n.length>=4&&(n[0].textContent=t,n[1].textContent=l,n[2].textContent=o,n[3].textContent=a)}function updateCalendarAndStatsForLesson(e,t){let l=document.querySelector(`.calendar-day[data-date="${e}"]`);l&&(l.classList.remove("calendar-day--none","calendar-day--paid","calendar-day--unpaid","calendar-day--pending"),l.classList.add("calendar-day--"+t));let o=document.querySelectorAll("#lessonsTableContainer tbody tr"),a=0,n=0,s=0,i=0;o.forEach(e=>{let t=JSON.parse(e.dataset.lesson);a++,"paid"===t.payment_status?n++:"unpaid"===t.payment_status?s++:"pending"===t.payment_status&&i++});let r=document.querySelectorAll(".stats-panel__value");r.length>=4&&(r[0].textContent=a,r[1].textContent=n,r[2].textContent=s,r[3].textContent=i)}function openLessonPageFromTable(e){let t=JSON.parse(e.dataset.lesson),l=e.dataset.studentId,o=e.dataset.studentName,a=parseInt(e.dataset.year),n=parseInt(e.dataset.month);openLessonPage(t,l,o,a,n)}function openAddLessonFromTable(e,t,l,o){let a=document.createElement("div");a.className="modal-overlay active",a.innerHTML=`
        <div class="modal">
            <button class="modal__close" onclick="this.closest('.modal-overlay').remove()">&times;</button>
    ...
    `),loadScheduleContent()}function switchScheduleMode(e){currentScheduleMode=e,"day"!==e&&(currentScheduleDate=new Date().toISOString().split("T")[0]),document.querySelectorAll(".schedule-mode-btn").forEach(e=>e.classList.remove("active")),event.target.classList.add("active"),loadScheduleContent()}function loadScheduleContent(){let e=document.getElementById("scheduleContent");if(e){if("day"===currentScheduleMode)loadDaySchedule(currentScheduleDate);else if("week"===currentScheduleMode)loadWeekSchedule(currentScheduleDate);else if("month"===currentScheduleMode){let[t,l]=currentScheduleDate.split("-");loadMonthSchedule(parseInt(t),parseInt(l))}}}function loadDaySchedule(e){fetch(`get_teacher_schedule.php?date=${e}&timezone=${TEACHER_TIMEZONE}`).then(e=>e.json()).then(t=>{let l=new Date(e).toLocaleDateString("ru-RU",{year:"numeric",month:"long",day:"numeric"}),o=`
        ...
        <div class="modal">
            ...
            <button class="btn btn--primary" onclick="submitReschedule(${e}, ${o}, '${a}', ${n}, ${s})">Перенести</button>
                ...
                </div>`,document.body.appendChild(o)})}function confirmStudentPicker(e){let t=e.closest(".modal-overlay");if(!t)return;let l=t.querySelectorAll("input[type=checkbox]:checked"),o={};l.forEach(e=>{let t=parseInt(e.value),l=e.closest("label").textContent.trim();o[t]=l});let a=selectedLessonStudents[0];a&&!o[a.id]&&(o[a.id]=a.name),selectedLessonStudents=Object.keys(o).map(e=>({id:parseInt(e),name:o[e]})),t.remove();let n=document.getElementById("selectedStudentsContainer");n&&(n.innerHTML=renderSelectedStudentsTags())}function renderStatistics(){setMainContent(`
        <div class="dashboard-header">
            <h2>Статистика</h2>
        </div>
        <div class="schedule-mode-switcher" style="margin-bottom:20px;">
            <button class="schedule-mode-btn active" id="statPaymentsTab" onclick="switchStatisticsTab('payments')">💰 Оплаты</button>
            <button class="schedule-mode-btn" id="statExamsTab" onclick="switchStatisticsTab('exams')">📝 Пробники</button>
        </div>
        <div id="statisticsContent"></div>
    `),loadPaymentStats()}function switchStatisticsTab(e){document.getElementById("statPaymentsTab").classList.toggle("active","payments"===e),document.getElementById("statExamsTab").classList.toggle("active","exams"===e),"payments"===e?loadPaymentStats():loadExamStudentsList()}async function loadPaymentStats(){let e=new Date().getFullYear(),t=await fetch(`get_payment_stats.php?year=${e}`),l=await t.json();if(!l.months){document.getElementById("statisticsContent").innerHTML='<div class="empty-state">Ошибка загрузки</div>';return}let o=["Январь","Февраль","Март","Апрель","Май","Июнь","Июль","Август","Сентябрь","Октябрь","Ноябрь","Декабрь"],a="";l.months.forEach(e=>{a+=`
            <tr>
                <td>${o[e.month-1]}</td>
                <td><span class="badge badge--paid">${e.paid} ₽</span></td>
                <td><span class="badge badge--unpaid">${e.unpaid} ₽</span></td>
                <td><span class="badge badge--pending">${e.pending} ₽</span></td>
            </tr>`}),a+=`
        <tr style="font-weight:700; background:#f9fafb;">
            <td>Итого за год</td>
            <td><span class="badge badge--paid">${l.total.paid} ₽</span></td>
            <td><span class="badge badge--unpaid">${l.total.unpaid} ₽</span></td>
            <td><span class="badge badge--pending">${l.total.pending} ₽</span></td>
        </tr>`,document.getElementById("statisticsContent").innerHTML=`
        <div class="table-responsive">
            <table>
                <thead><tr><th>Месяц</th><th>Оплачено</th><th>Не оплачено</th><th>Ожидается</th></tr></thead>
                <tbody>${a}</tbody>
            </table>
        </div>`}async function loadExamStudentsList(){let e=await fetch("get_exam_results.php"),t=await e.json();if(!Array.isArray(t)||0===t.length){document.getElementById("statisticsContent").innerHTML='<div class="empty-state">Нет учеников с пробниками</div>';return}let l="";t.forEach(e=>{let t=e.avg_score?Number(e.avg_score).toFixed(1):"—";l+=`<tr style="cursor:pointer;" onclick="loadExamResultsForStudent(${e.id}, '${e.first_name} ${e.last_name}')">
            <td>${e.first_name}</td>
            <td>${e.last_name||""}</td>
            <td>${e.exams_count}</td>
            <td>${t}</td>
        ...
        <div class="table-responsive">
            <table>
                <thead><tr><th>Имя</th><th>Фамилия</th><th>Пробников</th><th>Средний балл</th></tr></thead>
                <tbody>${l}</tbody>
            </table>
        </div>`}async function loadExamResultsForStudent(e,t){let l=await fetch(`get_exam_results.php?student_id=${e}`),o=await l.json();if(!Array.isArray(o)){document.getElementById("statisticsContent").innerHTML='<div class="empty-state">Ошибка загрузки</div>';return}let a=o.length?(o.reduce((e,t)=>e+Number(t.score),0)/o.length).toFixed(1):"—",n="";o.forEach(t=>{n+=`
            <tr>
                <td>${t.exam_name}</td>
                <td>${t.exam_date||"—"}</td>
                <td>${t.comment||""}</td>
        ...
        <button class="btn-back" onclick="loadExamStudentsList()">← Назад к списку учеников</button>
        <div class="dashboard-header">
            <h3>Пробники: ${t}</h3>
            <button class="btn-add" onclick="openAddExamModal(${e})">+ Добавить пробник</button>
        </div>
        <div class="table-responsive">
            <table>
        ...
        <div class="stats-panel" style="margin-top:16px;">
            <div class="stats-panel__item"><div class="stats-panel__label">Средний балл</div><div class="stats-panel__value">${a}</div></div>
        </div>`}function openAddExamModal(e){let t=document.createElement("div");t.className="modal-overlay active",t.innerHTML=`
        <div class="modal">
            <button class="modal__close" onclick="this.closest('.modal-overlay').remove()">&times;</button>
